...
**🛡️ 完全离线 &nbsp;·&nbsp; ⚡ GPU 实测最高 ≈15,832× &nbsp;·&nbsp; 🗂️ 10 种格式 &nbsp;·&nbsp; 🧠 字典 + 规则双引擎 &nbsp;·&nbsp; ⏸️ 断点续跑 &nbsp;·&nbsp; 🔁 智能去重 &nbsp;·&nbsp; 🌐 中英双语**
...
开启 GPU 加速（可选组件 hashcat）后，PDF AES-256（R5）实测 GPU 峰值 **1,034,600,000/s**，最高提速 **≈15,832 倍**。全部数据来自产品真实破解通道（非独立基准程序）。

| 格式 | 加密方式 | CPU 计算（Intel Ultra 9 285H） | GPU 计算（Intel Arc 140T 核显） | GPU 加速 |
|---|---|---|---|---|
| docx / xlsx / pptx | Agile AES-256（SHA-512×10 万轮） | 989/s | 2,236/s | 2.3× |
| docx / xlsx / pptx | Standard AES-128（SHA-1×5 万轮，WPS 常见） | 1,638/s | 64,923/s | 40× |
| doc / xls / ppt | 旧式 RC4（MD5） | 92,594/s | 85,764,700/s | 926× |
| doc / xls / ppt | 旧式 RC4-40/128（SHA-1） | 91,053/s | 83,095,500/s | 912× |
| pdf | RC4-40（R2，Acrobat 3） | 69,693/s | 95,932,100/s | 1,376× |
| pdf | RC4-128（R3，Acrobat 5） | 19,453/s | 5,181,900/s | 266× |
| pdf | AES-128（R4，Acrobat 7） | 18,563/s | 5,185,600/s | 279× |
| pdf | AES-256（R5，Acrobat 9） | 65,339/s | 1,034,600,000/s | 15,832× |
| pdf | AES-256（R6，Acrobat X+，最强） | 1,473/s | 4,202/s | 2.9× |
| zip | ZipCrypto（传统加密） | 7,291/s | 仅 CPU | — |
| zip | WinZip AES-128 | 23,794/s | 1,275,700/s | 54× |
| zip | WinZip AES-192 | 16,773/s | 1,226,100/s | 73× |
| zip | WinZip AES-256 | 12,427/s | 1,224,500/s | 98× |
| 7z | AES-256（LZMA2 压缩） | 1,124/s | 3,108/s | 2.8× |
| 7z | AES-256（仅存储） | 867/s | 2,996/s | 3.5× |
| rar | RAR5 内容加密 | 3,058/s | 16,447/s | 5.4× |
| rar | RAR5 头加密（-hp） | 2,909/s | 16,671/s | 5.7× |

> 测试环境：ThinkPad T14P Gen3 · Intel Core Ultra 9 285H · Intel Arc 140T 核显 · hashcat v7.1.2 · Python 3.13。
> \* 旧式 Office 校验只读取文件头部定长数据，速度与文件大小无关。
>
> 要点：① 破解速度由密钥派生算法决定，而非密码算法本身——同为 AES-256，PDF R5 与 R6 相差 24 万倍；② GPU 加速普遍 30～1,000 倍，本表仅用核显实测，高性能独显会有极大提升；③ Office/PDF/RAR/7z 的速度与文件大小无关，ZipCrypto 随体积增大而变慢，7z 慢是因 52 万轮强密钥派生；④ ZipCrypto 当前仅启用 CPU 通道——GPU 校验存在误判风险，为保证结果准确。
...
| CPU 太慢等不起 | 一键启用 GPU 加速（可选组件 hashcat），全格式实测提速 2.3×～15,832× |
